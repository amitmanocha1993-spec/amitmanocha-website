import type { Metadata } from 'next';
import PageStructuredData from '@/components/PageStructuredData';

export const metadata: Metadata = {
  title: 'Home',
  description: 'Welcome to the portfolio of Amit Manocha (A.K.A. Aziim Dehlvi) - Creative designer, author, and poet specializing in cinematic design, Urdu poetry, and literary works.',
  alternates: {
    canonical: 'https://www.amitmanocha.co.in',
  },
  openGraph: {
    title: 'Home | Amit Manocha',
    description: 'Welcome to the portfolio of Amit Manocha (A.K.A. Aziim Dehlvi) - Creative designer, author, and poet specializing in cinematic design, Urdu poetry, and literary works.',
    url: 'https://www.amitmanocha.co.in',
  },
  twitter: {
    title: 'Home | Amit Manocha',
    description: 'Welcome to the portfolio of Amit Manocha (A.K.A. Aziim Dehlvi) - Creative designer, author, and poet specializing in cinematic design, Urdu poetry, and literary works.',
  },
};

export default function HomeLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <>
      <PageStructuredData
        pageName="Home"
        pageUrl="https://www.amitmanocha.co.in"
        pageDescription="Welcome to the portfolio of Amit Manocha (A.K.A. Aziim Dehlvi) - Creative designer, author, and poet specializing in cinematic design, Urdu poetry, and literary works."
        breadcrumbs={[
          { name: 'Home', url: 'https://www.amitmanocha.co.in' },
        ]}
      />
      {children}
    </>
  );
}
